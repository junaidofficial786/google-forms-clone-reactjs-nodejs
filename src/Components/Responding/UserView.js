...
function UserView(props) {
  ...
  const handleRadioChange = (e, i) => {
    // console.log(optionValue[i].option, "Wow");
    var questionId = questions[i]._id;
    var optionId = questions[i].options[e.target.value]._id;
    ...
    var finalChanged = optionValue.map((option, index) => {
      if (index === i) {
        return { option: Number(e.target.value), question: option.question };
      } else {
        return option;
      }
    });
      ...
      (data) => {
        ...
        setOptionValue(
          data.questions.map((question) => {
            return { option: 0, question: question._id };
          })
        );
  ...
  useEffect(() => {
    // check email pattern
    // if valid then set emailError to false
    // else set emailError to true
